...
def date_converter(language, date):
    first_column = date.find('/')
    second_column = date.find('/', first_column + 1)
    day = date[first_column + 1:second_column]
    month = date[:first_column]
    year = date[second_column + 1:]

    dates = day + ' ' + language[int(month)] + ' ' + year
    return dates
